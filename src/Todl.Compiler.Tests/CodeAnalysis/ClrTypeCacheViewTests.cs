...
public sealed class ClrTypeCacheViewTests
{
    ...
    {
        var syntaxTree = SyntaxTree.Parse(SourceText.FromString(importDirective), new());
        var resolvedType = TestDefaults.DefaultClrTypeCache.CreateView(syntaxTree.Directives.OfType<ImportDirective>()).ResolveBaseType(typeName);
        resolvedType.ClrType.AssemblyQualifiedName.Should().Be(targetType.AssemblyQualifiedName);
